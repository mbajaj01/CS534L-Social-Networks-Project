...
class InfGraph:public Graph
{
    public:
        ...
        bool isEnvironment;
        vector<double> itemDistribution;
        InfGraph(string folder, string graph_file, int topics):Graph(folder, graph_file, topics){
            hyperG.clear();
            for(int i=0; i<n; i++)
            ...
            while(!q.empty()) {
                ...
                if(influModel==IC){
                    ...
                    for(int j=0; j<(int)gT[i].size(); j++){
                        ...
                        double probToCompare;
                        if (isEnvironment){
                            probToCompare = dotProduct(itemDistribution, probT[i][j]);
                        }else{
                            probToCompare = dotProduct(itemDistribution, probTEstimate[i][j]);
                        }
                        if(randDouble > probToCompare)
                            continue;
                        if(visit[v])
                            continue;
                        if(!visit[v])
                    ...
                    for(int i=0; i<(int)gT[expand].size(); i++){
                        if (isEnvironment){
                            ...
                            ASSERT( i< (int)probTEstimate[expand].size());
                        }
                        if (isEnvironment){
                            randDouble -= dotProduct(itemDistribution, probT[expand][i]);
                        }else{
                            randDouble -= dotProduct(itemDistribution, probTEstimate[expand][i]);
                        }
                        if(randDouble>0)
                            continue;
                        //int u=expand;
